    ...
    */
                    ...
                    if (resources == null) {
                        ...
                        for (TypeElement typeElement : entityElement.typeElements) {
                            ...
                            if(domainNamedGraph!=null && !domainNamedGraph.isEmpty()){
                                ...
                                this.modelOutput.getQuadGraph().add(new ResourceImpl(X3ML.DomainElement.namedGraphProduced).asNode(), 
                            ...
                            if(namedGraph!=null){
                                this.modelOutput.getQuadGraph().add(new ResourceImpl(namedGraph).asNode(),
                                        new ResourceImpl(generatedValue.text).asNode(), 
                                        new ResourceImpl("http://www.w3.org/1999/02/22-rdf-syntax-ns#type").asNode(),
                                        new ResourceImpl(modelOutput.getNamespace(typeElement)).asNode());
                            }
                ...
                if (additionalEntityResolver.get(i).hasResources()) {
                    for (Resource resource : additionalEntityResolver.get(i).resources) {
                        ...
                        if(X3ML.Mapping.namedGraphProduced!=null && !X3ML.Mapping.namedGraphProduced.isEmpty()){
                            this.modelOutput.getQuadGraph().add(new ResourceImpl(X3ML.Mapping.namedGraphProduced).asNode(),
                                                      fromResource.asNode(), 
    ...
    private class LabelNode {
        ...
        public void linkFrom(Resource fromResource, Derivation derivedBy) {
            ...
            if(X3ML.Mapping.namedGraphProduced!=null && !X3ML.Mapping.namedGraphProduced.isEmpty()){
                EntityResolver.this.modelOutput.getQuadGraph().add(new ResourceImpl(X3ML.Mapping.namedGraphProduced).asNode(),
                                        fromResource.asNode(), 
                                        property.asNode(),
                                        literal.asNode());
            }if(X3ML.DomainElement.namedGraphProduced!=null && !X3ML.DomainElement.namedGraphProduced.isEmpty() && derivedBy==Derivation.Domain){
               EntityResolver.this.modelOutput.getQuadGraph().add(new ResourceImpl(X3ML.DomainElement.namedGraphProduced).asNode(),
                                        fromResource.asNode(), 
                                        property.asNode(),
                                        literal.asNode());
                ...
                if(X3ML.LinkElement.namedGraphProduced!=null && !X3ML.LinkElement.namedGraphProduced.isEmpty()){
                   EntityResolver.this.modelOutput.getQuadGraph().add(new ResourceImpl(X3ML.LinkElement.namedGraphProduced).asNode(),
                                        fromResource.asNode(), 
                                        property.asNode(),
                                        literal.asNode());
                }
