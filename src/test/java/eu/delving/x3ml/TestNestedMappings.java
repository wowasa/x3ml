 ...
 */
public class TestNestedMappings {
    private final Logger log = Logger.getLogger(getClass());
    private final Generator VALUE_POLICY = X3MLGeneratorPolicy.load(null, X3MLGeneratorPolicy.createUUIDSource(1));

    @Test
    public void testEmptyElement() {
        X3MLEngine engine = engine("/nested_mappings/mappings.x3ml");
        X3MLEngine.Output output = engine.execute(document("/nested_mappings/input.xml"),VALUE_POLICY);
        String[] mappingResult = output.toStringArray();
        String[] expectedResult = xmlToNTriples("/nested_mappings/expectedResult.rdf");
